...
from models.tree_based_retrieval import TreeModel
import os
import time
...
def train():
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        tf.gfile.Copy(FLAGS.input_previous_model_path + "/" + FLAGS.tree_index_file, FLAGS.output_model_path + "/" + FLAGS.tree_index_file, overwrite=True)
        ...
        with tf.Session(config = config) as session:
            ...
            while True:
                try:
                    ...
                    if step % FLAGS.checkpoint_frequency == 1:
                        ...
                        if trainer.improved():
                            saver.save(session, FLAGS.output_model_path + "/tree_model", global_step=step)
            ...
            if not trainer.early_stop():
                saver.save(session, FLAGS.output_model_path + "/" + "tree_model_final", global_step = step)
